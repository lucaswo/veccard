...
if __name__ == '__main__':
    ...
    for run in range(config["runs"]):
        ...
        this_run_qerrors = q_loss_np(y_test, y_pred)
        qerror = np.mean(this_run_qerrors)
        all_qerror.append(this_run_qerrors)
    ...
    np.savetxt("all_qerror.csv", np.asarray(all_qerror), delimiter="\n")
    ...
    # test try to read saved_model
    loaded_model = saved_model.load(config["model_file"])
    print(loaded_model)
